export const findIndexOfPerson = (arr, occupant) =>
  arr.findIndex((person) => person.x === occupant.x && person.y === occupant.y);
...
export const getClosestEl = (el, arr) =>
  arr.reduce((prev, curr) =>
    Math.abs(curr - el) < Math.abs(prev - el) ? curr : prev
  );

export const getClosestPoint = (point, arr) =>
  arr.reduce((prev, curr) =>
    Math.abs(distance(curr, point)) < Math.abs(distance(prev, point))
      ? curr
      : prev
  );
