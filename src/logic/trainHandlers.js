...
export const peopleToAdd = (stop, train) =>
  Math.floor(
    desiredOccupancy(stop, maxOccupancy) - getOccupancy(train, height, width)
  );
export const peopleToRemove = (stop, train) =>
  Math.floor(stop[2] * getOccupancy(train, height, width));
...
export function scanTrain(train, height, width) {
  ...
  const { availableSeats, availableSpaces } = getAvailableSeats(
    newGridTrain,
    height,
    width
  );
  const { occupiedSpaces, occupiedAreas } = getOccupation(
    newGridTrain,
    height,
    width
  );
...
export const handleEgress = (
  ...
  for (var i = 0; i < toRemove; i++) {
    ...
    var boardedIdx = findIndexOfPerson(
      boarded,
      occupiedAreas[randIdx].occupant
    );
...
export const handleMoveSeats = (
  ...
  while (availableSeats.length && occupiedSpaces.length) {
    ...
    var closestSeat = getClosestPoint(
      occupiedSpaces[randSpaceIdx],
      availableSeats
    );
    var idxToRemove = findIndexOfPerson(availableSeats, {
      x: closestSeat.x,
      y: closestSeat.y,
    });
    ...
    var boardedIdx = findIndexOfPerson(
      boarded,
      occupiedSpaces[randSpaceIdx].occupant
    );
    var totalIdx = findIndexOfPerson(
      total,
      occupiedSpaces[randSpaceIdx].occupant
    );
