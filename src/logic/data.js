...
export const stops = [
  ...
  ["Broadway Jctn", 0.3579952267, 0.3],
...
// enter
export const enterFn = (enter, chartType) =>
  enter
    .append("circle")
    ...
    .attr(
      "cx",
      (d) =>
        introduceNoise(getClosestEl(d.x, C.doorIdxs) * C.squareSize) +
        C.squareSize / 2
    )
    ...
    .attr("stroke", "black")
    // .style('filter', 'drop-shadow(1px 2px 4px black)')
    .attr("opacity", 0)
    .transition()
    .duration(250)
    .attr("opacity", 1)
...
export const exitFn = (exit) =>
  exit
    ...
    .attr(
      "cx",
      (d) => introduceNoise(getClosestEl(d.x, C.doorIdxs) * C.squareSize) + 30
    )
    .attr("cy", introduceNoise((C.height * C.squareSize) / 2))
    .transition()
    .duration(600)
...
export const chartSeries = (keys, stack, isProp) => {
  const stacked = d3.stack().keys(keys);
...
// Colors
export const raceColors = d3
  .scaleOrdinal(d3.schemeCategory10)
  .domain(C.raceKeys);

export const incomeColors = d3
  .scaleOrdinal(d3.schemeRdYlGn[11])
  .domain(C.incomeKeys);
...
export const chartTypeInfo = {
  ...
  income: {
    ...
    axisTitle: (type) =>
      type === C.standard
        ? "Amount of People by Income"
        : "Proportion of People by Income",
  },
};
...
export const transitionColors = (selection, chartType) =>
  selection
    .transition()
    .duration(1000)
    .attr("fill", (d) => chartTypeInfo[chartType].colors(d[chartType]));
